const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

async function fixRepositoryUser() {
  try {
    // Get the repository without userId
    const repo = await prisma.repository.findFirst({
      where: { userId: null }
    });

    if (!repo) {
      console.log('No repository without userId found.');
      return;
    }

    console.log(`Found repository: ${repo.owner}/${repo.name} (ID: ${repo.id})`);

    // Get the user (assuming there's only one user)
    const user = await prisma.user.findFirst();

    if (!user) {
      console.log('No user found in the database. Creating a new user...');

      // Create a user with the information from the repository
      const newUser = await prisma.user.create({
        data: {
          githubId: repo.githubId,
          username: repo.owner,
          avatarUrl: `https://github.com/${repo.owner}.png`,
          accessToken: 'dummy-access-token',
        }
      });

      console.log(`✅ Created user: ${newUser.username} (ID: ${newUser.id})`);

      // Update the repository with userId
      await prisma.repository.update({
        where: { id: repo.id },
        data: { userId: newUser.id }
      });

      console.log(`✅ Updated repository ${repo.owner}/${repo.name} with userId ${newUser.id}`);
    } else {
      console.log(`Found user: ${user.username} (ID: ${user.id})`);

      // Update the repository with userId
      await prisma.repository.update({
        where: { id: repo.id },
        data: { userId: user.id }
      });

      console.log(`✅ Updated repository ${repo.owner}/${repo.name} with userId ${user.id}`);
    }
  } catch (error) {
    console.error('❌ Error:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fixRepositoryUser();
