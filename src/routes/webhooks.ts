 ...
 */
// Function to create logs for existing issues
async function createLogsForExistingIssues(repo: any) {
  console.log(`\n🔍 Creating logs for existing issues in ${repo.owner}/${repo.name}...`);
  console.log('Repository ID:', repo.id);
  console.log('Repository GitHub ID:', repo.githubId);
  console.log('Repository user data:', JSON.stringify(repo.user, null, 2));
  
  if (!repo.user || !repo.user.accessToken) {
    console.log("Repository is not associated with a user. Skipping GitHub API calls.");
    console.log("Repository object:", JSON.stringify(repo, null, 2));
    return;
  }
  ...
  try {
    ...
    console.log(`Fetching issues from GitHub API for ${repo.owner}/${repo.name}`);
    console.log(`Using access token: ${repo.user.accessToken.substring(0, 10)}...`);
    ...
    try {
      ...
      console.error('Error fetching issues from GitHub:', apiError);
    ...
    for (const issue of issues) {
      ...
      console.log(`Issue state: ${issue.state}, Issue URL: ${issue.html_url}`);
      
      // Check if log already exists
      console.log(`Checking for existing log with title: ${issue.title}`);
    ...
    console.log(`\n✅ Finished processing all issues. Processed ${issues.length} issues.`);
    
    // Count how many logs were created
    const allLogs = await prisma.botActionLog.count({
      where: { repositoryId: repo.id }
    });
    console.log(`Total logs in database for this repository: ${allLogs}`);
  } catch (error) {
...
async function processWebhookEvent(repo: any, eventType: string, payload: any, deliveryId: string) {
  ...
  for (const rule of matchedRules) {
    ...
    try {
      ...
      if (actions.labels && Array.isArray(actions.labels)) {
        console.log(`Applying labels: ${actions.labels.join(', ')}`);
        try {
          ...
          actionsTaken.push(`Applied labels: ${actions.labels.join(', ')}`);
        } catch (labelError) {
          console.error('❌ Label application failed:', labelError);
          errors.push(`Label application failed: ${labelError}`);
        }
      }

      // Add comment if specified
      if (actions.comment) {
        console.log(`Adding comment: ${actions.comment}`);
        try {
          ...
          actionsTaken.push(`Added comment: ${actions.comment.substring(0, 50)}...`);
        } catch (commentError) {
          console.error('❌ Comment creation failed:', commentError);
          errors.push(`Comment creation failed: ${commentError}`);
        }
      }

      // Assign to user if specified
      if (actions.assignee) {
        console.log(`Assigning to: ${actions.assignee}`);
        try {
          await octokit.rest.issues.addAssignees({
            owner: repo.owner,
            repo: repo.name,
            issue_number: number || 0,
            assignees: [actions.assignee],
          });
          actionsTaken.push(`Assigned to ${actions.assignee}`);
        } catch (assignError) {
          console.error('❌ Assignment failed:', assignError);
          errors.push(`Assignment failed: ${assignError}`);
        }
      }

      // Set milestone if specified
      if (actions.milestone) {
        console.log(`Setting milestone: ${actions.milestone}`);
        try {
          await octokit.rest.issues.update({
            owner: repo.owner,
            repo: repo.name,
            issue_number: number || 0,
            milestone: actions.milestone,
          });
          actionsTaken.push(`Set milestone: ${actions.milestone}`);
        } catch (milestoneError) {
          console.error('❌ Milestone setting failed:', milestoneError);
          errors.push(`Milestone setting failed: ${milestoneError}`);
        }
      }

      // Close issue if specified
      if (actions.close) {
        ...
        try {
          ...
          actionsTaken.push('Closed issue');
        } catch (closeError) {
          console.error('❌ Issue closing failed:', closeError);
          errors.push(`Issue closing failed: ${closeError}`);
        }
      }

      // Notify Slack if configured
      if (actions.slack && process.env.SLACK_WEBHOOK_URL) {
        ...
        try {
          await fetch(process.env.SLACK_WEBHOOK_URL, {
            ...
            body: JSON.stringify({
              text: `GitHub ${eventType} action triggered`,
              blocks: [
                {
                  type: 'section',
                  text: {
                    type: 'mrkdwn',
                    text: `*${repo.name}*
${eventType} ${eventAction} by ${author}`,
                  },
                },
                {
                  type: 'section',
...
export function verifySignature(secret: string | undefined, body: string, signatureHeader: string | undefined): boolean {
